...
function fetchPokemonData() {
  ...
  const pokemons = JSON.parse(fetchPokemonData());
  const sprites = Object.values(pokemons['sprites']);
    const piclist = sprites.filter(i => i != null);

const makeHtmlLists = piclist.map(i => {
  let pic;
  let picset =document.createElement('li')
  pokemonImage = document.createElement('img');
  pokemonImage.src = i;
  pokemonImage.height = '400';
  pokemonImage.width = '200';
  
  picset.appendChild(pokemonImage)
  document.body.appendChild(picset);
});
